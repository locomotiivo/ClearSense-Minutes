...
class FilesVC: UIViewController {
    ...
    @IBAction func onClickMinute(_ sender: UIButton) {
        guard let cell = tableView.cellForRow(at: IndexPath(row: sender.tag, section: 0)) as? FileRow,
              let vc = self.storyboard?.instantiateViewController(identifier: "MinuteVC") as? MinuteVC,
